...
import { syncShopifyCatalogData } from "@/lib/application/shopify-catalog-sync-service";
...
/** One Shopify products page per call — keep under Vercel limits. */
...
/**
 * POST /api/commerce/sync/shopify-products
 * Body: { cursor?: string | null, lockToken: string, mode?: "incremental" | "full" }
 *
 * Pulls Shopify products/variants into Aarla catalog.
 * Does NOT write stock_movements or invent inventory balances.
 */
export async function POST(request: Request) {
  try {
    ...
    const data = await syncShopifyCatalogData({
