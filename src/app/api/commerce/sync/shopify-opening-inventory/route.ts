...
import { syncShopifyOpeningInventory } from "@/lib/application/shopify-opening-inventory-service";
...
/**
 * POST /api/commerce/sync/shopify-opening-inventory
 * One-time legacy base inventory: Shopify available qty → Studio opening receipts.
 * Does not continuously sync Shopify inventory thereafter.
 */
export async function POST(request: Request) {
  try {
    ...
    const data = await syncShopifyOpeningInventory({
