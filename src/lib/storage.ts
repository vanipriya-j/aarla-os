...
import { peopleSeed, registrationsSeed } from "./network-data";
...
export function useNetworkStore() {
  ...
  const persistRegs = useCallback((next: ProductRegistration[]) => {
    writeJson(REGS_KEY, next);
  }, []);
  ...
  const registerProduct = useCallback(
    (input: RegisterInput) => {
      ...
      persistRegs([reg, ...currentRegs]);
      return { user, registration: reg };
    },
    [persistPeople, persistRegs],
  );
