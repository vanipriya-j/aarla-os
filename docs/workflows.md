...
- **Live commerce funnel (PR 8)** — when LIVE (panel always visible): funnel counts from `commerce_events` (product viewed → purchased) plus demand units per variant from `cart_sessions` (active / checkout / identified abandoned / anonymous abandoned). Inventory is **not** reduced by these units.
- **Schema** — also folded into `supabase/aarla-os-complete.sql` (single concatenated schema for the final clean setup). Append migrations via `npm run schema:complete`.
...
- **Live cart funnel (PR 8)** — Shopify Web Pixel / Custom Pixel → `POST /api/integrations/shopify/commerce-events` → `commerce_events` + `cart_sessions`. Demand signal for Customer Calls **Live carts** and campaign **Commerce funnel**. Does **not** write `stock_movements` or soft reserves. Extends abandoned-checkout Admin sync; see `docs/shopify-web-pixel.md`. After PR 8 merge → one clean setup (demo off) or `aarla-os-complete.sql`; deploy pixel; re-sync Shopify.
